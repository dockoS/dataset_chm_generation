...
def get_one_satelite_image(area,bbox,start_date="2023-02-01",end_date="2023-04-01",satellite_id="s2_l2a",crs_input="4326",crs_output="3857"):
    ...
    for _date in tqdm(get_dates(start_date,end_date),desc=f'searching images for satelites= {satellite_id}'):
        ...
        params = f'service=WCS&request=GetCoverage&version=2.0.0&coverageId={sat_id}&format={image_format}' \
                f'&subset=Long({long})&subset=Lat({lat})&subset=time("{_date}")&subsettingCrs=EPSG:{crs_input}' \
                f'&outputCrs=EPSG:{crs_output}&scaleaxes=x({scale_factor}),y({scale_factor})&cloud_cover={cloud_cover_percentage}'
...
def add_physic_parameter_to_df_bis():
    pass
...
"""255, 255, 102)
Jaune vif : RGB (255, 204, 0)
Jaune-vert : RGB (154, 205, 50)
Vert citron : RGB (173, 255, 47)
Vert forêt : RGB (34, 139, 34)"""
...
def create_tiff(polygon,filename,zoom=18):
...
def get_statistics(polygon,df_physic_parameter,type):
    p=Polygon(polygon)
  
    #area=p.transform(6933, clone=True).area/10000
    area=compute_area(p)/10000
    nbr_trees=len(df_physic_parameter) 
    qte_totale=sum(list(df_physic_parameter[f"{type}"]))

    #densite by hectare
    densite=qte_totale//area
    #number of credit carbon
    if type=="carbon":
        nbr_credit=qte_totale/1000
        return {"nbr_trees":nbr_trees,"qte_totale":qte_totale,"densite":densite,"nbr_credit":nbr_credit}
    elif type=="biomass":
        return {"nbr_trees":nbr_trees,"qte_totale":qte_totale,"densite":densite}
    else:
        raise ValueError("type not valid")
if __name__ == "__main__":


    # Your GeoJSON content
    data = {
    ...
    gdf = gpd.GeoDataFrame.from_features(data["features"], crs=data["crs"]["properties"]["name"])

    # Reproject to EPSG:4326
    gdf_4326 = gdf.to_crs(epsg=3857)

    # Print the polygon in EPSG:4326
